...
<body>
    <div id="wrap">
        <nav>
            ...
            <!-- <button type="button" onclick="location.href='http://pineit.co.kr/daeji/mgtRpt.html'" style="display: flex; justify-content: center; align-items: center; padding: 8px 14px; border-radius: 1rem; background-color: rgba(58, 78, 132, 0.15);">
                MIS
            </button> -->
            
            <ul>
                <li onclick="location.href='mgtRpt'">경영보고</li>
                <li class="active" onclick="location.href='prodRpt'">생산보고</li>
        ...
        <section id="textWrapper">
            ...
            <div id="" class="section-contents scrolling-table common-table-wrap">
                <table id="" class="common-data-table amount-table" style="border-block: 0;">
                    ...
                    <tbody>
                    	<c:forEach  var = "pd" items="${prodList}">
                    		<c:set var = "gu" value = "${pd.daegubun}"/>
                    		<c:choose>
                    			<c:when  test="${gu eq '소계'}">
							 		<tr class = "subtotal-row">
	                    				<td style = "text-align:center">${pd.daegubun}</td>
	                    				<td style = "text-align:left">${pd.gubun1}</td>
	                    				<td><fmt:formatNumber value="${pd.landedweight}" pattern="###,###.#" /></td>
	                    				<td><fmt:formatNumber value="${pd.loss}" pattern="###,###.#" /></td>
	                    				<td><fmt:formatNumber value="${pd.acceptweight}" pattern="###,###.#" /></td>
	                    				<td><fmt:formatNumber value="${pd.diffweight}" pattern="###,###.#" /></td>
	                    			</tr>  
								</c:when>
...
<script>
	function submitForm() {
        ...
        if(sYmdFrom == '' || sYmdTo == ''){
    	    ...
    	    $.ajax({
    	        url: "/exe/prodRpt/prodlist", // 실제 컨트롤러의 엔드포인트
    	        method: "POST",
